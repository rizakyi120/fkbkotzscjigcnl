...
def pc(text):
    url = "http://192.168.1.106:5000/data"  # استبدل 192.168.1.117 بعنوان IP الخاص بجهاز الكمبيوتر الخاص بك
    headers = {'Content-Type': 'text/plain'}  # تحديد نوع المحتوى كنص عادي

    try:
        response = requests.post(url, data=text, headers=headers)
        print("Server response:", response.text)  # عرض النص كما هو
    except requests.exceptions.RequestException as e:
...
print(R)
print(" ============================= ==============")
print(G ,"Successfully Has ", P ,"Activated" , Z , "PEGASUS BOT 10")
print(R, "============================= ==============")
print(R)
#    time.sleep(6)
 #   os.system("clear")
...
#                    clientC.connect((host, port))
                ...
                if remote in r:
                    ...
                    if '0e00' in dataS.hex()[:4] and '0e15' in dataC.hex()[:4] and preventlag == True:
                        ...
                        if b"@FNM" in dataS and comand == True:
                        ...
                        if b"@FPC" in dataS and comand == True:
                            text = str(bytes.fromhex(dataS.hex()))
                            pattern = r'@FPC(\d+)'
                            match = re.search(pattern, text)
                            number = match.group(1)
                        ...
                        if b"PEGS-MA91-A7QK-82QL" in dataS:
                            ...
                            if "fff64d" in mack:
                                ...
                                comand = False
                                fadai_msg(f"{mack}", dataS.hex(), client)
                        if b"@FKN" in dataS and comand ==True:
                            for i in range (20):
                                time.sleep(1.5)
                        ...
                        if b"@FDI" in dataS and comand == True:
                            fadai_msg(f"""[b][c][ffe75c]تم التفعيل""", dataS.hex(), client)
                            invit_spam =True
                        if b"@FMH" in dataS and comand == True:
                        	fadai_msg("[b][c][00ffff]جاري إستدعاء المحكمة\n[0000FF]تنظيم الجلسة\n[FF00FF]انتضار المتهم....", dataS.hex(), client)
                        	inf = info(dataS.hex())
                        ...
                        if b'@FDX' in dataS and comand ==True:
                            time.sleep(2)
                            clientC.send(xspy)
                        ...
                        if b"@FDRM" in dataS and comand == True:
                           try:
                                ...
                                pass
                        if b"@FPS" in dataS and comand ==True:
                                try:
                                    ...
                                    threading.Thread(target=send_msg, args=(client, dataS.hex(), f"""[b][c][FF00FF]PEGA[00FFFF]SUS V10""", 0.31)).start()
                                except:
                                        threading.Thread(target=send_msg, args=(client, dataS.hex(), f"""ERROR !""", 0.31)).start()
                        if b"@FNR" in dataS and comand == True:
                            threading.Thread(target=lagroom,args=(clieee,lag)).start()

                        if b"@FYT" in dataS and comand ==True:
                            add_yout =True
                        ...
                        if add_yout == True:
                            ...
                            try:
                                ...
                                pass
                        if b"@FWE" in dataS and comand ==True:
                        ...
                        if b"@FDC" in dataS and comand ==True:
                            text = str(bytes.fromhex(dataS.hex()))
                            pattern = r'@FDC(\d+)'
                            match = re.search(pattern, text)
                            number = match.group(1)
                            ...
                            fadai_msg(f"""[b][c]{color()}تم التفعيل""", dataS.hex(), client)
                            clientC.send(bytes.fromhex(dan))
                        if b"@FDE" in dataS and comand ==True:
                            text = str(bytes.fromhex(dataS.hex()))
                            pattern = r'@FDE(\d+)'
                            match = re.search(pattern, text)
                            number = match.group(1)
                        ...
                        if b"@FGE" in dataS and comand ==True:
                            text = str(bytes.fromhex(dataS.hex()))
                            pattern = r'@FGE(\d+)'
                            match = re.search(pattern, text)
                            number = match.group(1)
                        ...
                        if b"@FKE" in dataS and comand ==True:
                        ...
                        if b'@FDM' in dataS and comand ==True:
                            fadai_msg(f"""[b][c][ffe75c]تم التفعيل""", dataS.hex(), client)
                            id = dataS.hex()[12:22]
                            clientC.send(bytes.fromhex(f"080000001308{id}100820022a0708a6b10318fa01"))
                        ...
                        if b'@FLG' in dataS and comand ==True:
                            text = str(dataS)
                            pattern = r'@FLG(\d+)'
                            match = re.search(pattern, text)
                            key = match.group(1)
...
رسالة إبداع
العودة
غير اسم""", dataS.hex(), client)
                            fadai_msg(f"""[b][c]إبهار :[f5f120]

رقص لاعب
سبام رقص عشوائي سكواد
سبام رقص واحدة سكواد
رقص نفسك عشوائي
سبام رقصاتك عشوائي
سبام رقصاتك واحدة
""", dataS.hex(), client)
                            fadai_msg(f"""[b][c]تحصيل اسلخة متطورة :
